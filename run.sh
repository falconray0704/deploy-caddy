...
deploy_caddy()
{
    ...
    then
        echoY "Already deployed caddy in ${DRAWIO_HOME}, check it there first!"
        exit 1
    else
        echoY "Deploying caddy..."
        cp -a ./src ${DRAWIO_HOME}
        echoG "caddy has been deployed to ${DRAWIO_HOME} successfully."
        cat ./src/README.md
    fi
}
